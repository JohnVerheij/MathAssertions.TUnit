...
- Test count: 645 (was 619 at 0.1.0); +12 new failure-message-rendering tests and +11 new axis-angle predicate tests in `MathAssertions.Tests`, plus +3 adapter end-to-end tests in `MathAssertions.TUnit.Tests` pinning the fluent `HasAxisAngleApproximately` chain (happy path, wrong-axis with extracted-diagnostics rendering, and SO(3) double-cover via a negated quaternion).
