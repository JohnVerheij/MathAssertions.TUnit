...
| Linear algebra | `Matrix4x4` invariants (symmetric, orthogonal, proper rotation, identity, determinant, trace, invertible); `Vector3` pair properties (orthogonality, scale-invariant parallelism, angle between, linear independence) |
...
**Requirements:** TUnit 1.61.15 or later, .NET 10. `MathAssertions` (the framework-agnostic core) and TUnit's runtime + assertion deps come transitively. The package is AOT-compatible, trimmable, and uses no runtime reflection in the assertion path.
...
See [CONTRIBUTING.md](CONTRIBUTING.md) for the full PR review checklist and API design principles, and [CONVENTIONS.md](CONVENTIONS.md) for the family-wide code conventions shared across `LogAssertions.TUnit`, `SnapshotAssertions.TUnit`, `TimeAssertions.TUnit`, `JsonAssertions.TUnit`, `SseAssertions.TUnit`, and this repo.
